...
- `terraform/lab/gitops/stack/` — GitOps-managed parallel stack:
  ...
  - `28-app-forgejo.yaml` — Argo CD app for Forgejo (with in-cluster PostgreSQL).
...
- Forgejo data and PostgreSQL are configured to use `forgejo-nfs` StorageClass
  (backed by `nfs-subdir-external-provisioner`).
